...
import { Suspense } from "react"; // 🔥 IMPORT JURUS DEWA
...
// ==========================================
// 🚀 ASYNC COMPONENTS (Biar Ngerender Paralel!)
// ==========================================

async function WealthWidget() {
  ...
  const stocks = await Promise.all(
    dbStocks.map(async (stock) => {
      try {
        ...
        return { ...stock, totalValue: livePrice * stock.lotQuantity * 100 };
      } catch {
...
async function ContentStatsWidget() {
...
async function AnalyticsWidget({ range, startDate }: { range: string, startDate: Date }) {
  ...
  return <DashboardChart chartData={chartData} currentRange={range} totalViews={logsInRanges.length} totalVisitors={visitorsMap.size} />;
}

async function ExpensesPieWidget() {
  const financialLogs = await prisma.financialLog.findMany({ where: { type: "OUT" } });
  ...
  return <ExpensePieChart data={pieChartData} />;
}

async function CalendarWidget() {
  const calendarEvents = await getGoogleCalendarEvents();
  return (
    <div className="p-6 overflow-y-auto bg-slate-50/50 hide-scrollbar h-[400px]">
...
// ==========================================
// 🎨 RENDER UI UTAMA (TANPA BLOCKING)
// ==========================================
export default async function AdminDashboard({
  ...
  return (
    <div className="space-y-8 w-full max-w-7xl pb-10">
      ...
      {/* 💰 SUPER CARD DIBUNGKUS SUSPENSE */}
      <Suspense fallback={<div className="w-full h-[260px] bg-slate-200 animate-pulse rounded-3xl"></div>}>
        <WealthWidget />
      </Suspense>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <Suspense fallback={<div className="w-full h-28 bg-slate-100 animate-pulse"></div>}>
          <ContentStatsWidget />
        </Suspense>
      </div>

      {/* 🔥 GRID: ANALYTICS & PIE CHART */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <Suspense fallback={<div className="w-full h-[300px] bg-slate-100 animate-pulse rounded-xl flex items-center justify-center text-slate-400 font-bold">Menyiapkan Grafik...</div>}>
            <AnalyticsWidget range={range} startDate={startDate} />
          </Suspense>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col min-h-[400px] overflow-hidden">
          ...
          <div className="p-4 flex-1 w-full h-full min-h-[300px]">
            <Suspense fallback={<div className="w-full h-full min-h-[300px] bg-slate-50 animate-pulse rounded-xl flex items-center justify-center">Loading Data...</div>}>
              <ExpensesPieWidget />
            </Suspense>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
          ...
          <Suspense fallback={<div className="h-[400px] flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#013880]"></div></div>}>
            <CalendarWidget />
          </Suspense>
        </div>
